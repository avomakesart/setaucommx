...
export const Footer = () => {
  ...
  legal: {
    copyright:
      `© ${new Date().getFullYear()} Setau. Todos los derechos reservados.`,
  },
};
      ...
      <div className="px-14 pt-16 pb-13">
        <div className="max-w-full w-full mx-auto lg:max-w-7xl">
          <div className="flex gap-8 mb-16 justify-between items-center">
            ...
            <div className="flex">
              <Link href='/contacto' className="bg-[white] text-foreground dark:text-accent text-center tracking-[-0.008em] whitespace-nowrap cursor-pointer justify-center items-center min-h-[3rem] text-base font-medium leading-normal transition-[background-color] duration-[0.2s,transform] delay-[0.2s] flex px-6 py-2 rounded-xl">
                Contactanos
              </Link>
            </div>
          </div>
            ...
            <div className="flex flex-col md:flex-row justify-between gap-20">
              ...
              <div className="flex flex-col md:flex-row gap-20">
                <div className="flex flex-col gap-2">
                  <span className="pl-3 text-background dark:text-foreground font-semibold">
                  ...
                  {footerData.links.items.map((item) => (
                    <Fragment key={item.label}>
                      <Link
                        href={item.href}
                        className="text-background dark:text-foreground whitespace-nowrap cursor-pointer justify-start items-center text-base font-light leading-normal flex p-3"
                      >
                        {item.label}
                      </Link>
                    </Fragment>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {footerData.contact.items.map((item) => (
                    <div
                      key={item}
                      className="text-background dark:text-foreground justify-start items-center text-base font-light leading-normal flex p-3"
                    >
                      {item}
                    </div>
                  ))}
        ...
        <div className="h-px bg-white mt-10" />
              <div className="text-center mt-10 text-white">{footerData.legal.copyright}</div>
      </div>
    </footer>
  );
};
